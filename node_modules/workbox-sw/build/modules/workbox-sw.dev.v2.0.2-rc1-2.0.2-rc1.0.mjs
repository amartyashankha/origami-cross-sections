...
const ERRORS={"not-in-sw":{message:'workbox-precaching must be loaded in your service worker file.'},"invalid-object-entry":{message:(a)=>`Precache entries entered as an object must have a 'url' and `+`'revision' parameter. The '${a.problemParam}' parameter has an `+`invalid value of '${a.problemValue}'.`},"invalid-string-entry":{message:(a)=>`Precache entries entered as strings must have at least `+`one character. Received url of '${a.url}'`},"invalid-request-entry":{message:(a)=>`Precache entry was coerced as a RequestCacheEntry but the type `+`received was not a Request. Received url of '${a.url}'`},"bad-cache-bust":{message:`The cache bust parameter must be a boolean.`},"duplicate-entry-diff-revisions":{message:(a)=>`An attempt was made to cache the same `+`url twice with each having different revisions. This is not `+`supported. First entry was: `+`'${JSON.stringify(a.firstEntry)}' and the second entry was `+`'${JSON.stringify(a.secondEntry)}'`},"request-not-cached":{message:(a)=>`The request fpr '${a.url}' failed the criteria to be `+`cached. By default, only responses with 'response.ok = true' `+`are cached.`},"requires-overriding":{message:'Method should be overridden by the extending class.'},"bad-cache-id":{message:(a)=>`The 'cacheId' parameter must be a string with at least `+`one character. Received '${JSON.stringify(a.cacheId)}'.`},"unexpected-precache-entry":{message:(a)=>`One of the precache entries had an unexpected type of: `+`'${typeof a.input}'`}};
